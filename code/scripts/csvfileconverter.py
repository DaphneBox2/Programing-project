...
fieldNames = ["countryCode2", "countryName", "age", "gender", "ruralUrban", "educationLevel", "fullTime", "children", "basicIncomeAwareness", "basicIncomeVote", "basicIncomeEffect", "basicIncomeArgumentsFor", "basicIncomeArgumentsAgainst", "ageGroup", "weight"]
...
for row in reader1:
	for j in range( 0, len( countries ) ):
		if row[0] == countries[j][1]:
			if row[0] != "":
				out = [row[0], countries [j][0], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10],  row[11],  row[12],  row[13],  row[14]]
				writer.writerow( out )
